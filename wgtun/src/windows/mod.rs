...
impl WireGuardNT {
    fn get_adapter_addresses(&self) -> Result<Vec<IpNet>> {
        // the adapter luid
        ...
        unsafe {
            ...
            debug!("list current adapter '{}' address", luid.Value);
            ...
            for family in [AF_INET, AF_INET6] {
                ...
                for i in 0..(*unicast_ip_address_table).NumEntries as usize {
                    ...
                    if (*unicast_ip_address_row).InterfaceLuid.Value == luid.Value {
                        let ip_net = if family == AF_INET {
                            let address_bytes =
                                (*unicast_ip_address_row).Address.Ipv4.sin_addr.S_un.S_un_b;
                            let ipv4_net = match Ipv4Net::new(
                                [
                                    address_bytes.s_b1,
                                    address_bytes.s_b2,
                                    address_bytes.s_b3,
                                ...
                                .into(),
                                (*unicast_ip_address_row).OnLinkPrefixLength,
                            ) {
                                Ok(v) => v,
                                Err(err) => {
                                    let _ = FreeMibTable(unicast_ip_address_table as *const _);
                                    bail!("convert ipv4 address: {err:#}");
                                },
                            };
                            IpNet::V4(ipv4_net)
                        } else {
                            let ipv6_net = match Ipv6Net::new(
                                (*unicast_ip_address_row)
                                    .Address
                                    .Ipv6
                                    .sin6_addr
                                    .u
                                ...
                                Err(err) => {
                                    let _ = FreeMibTable(unicast_ip_address_table as *const _);
                                    bail!("convert ipv6 address: {err:#}");
                                },
                            };
                            IpNet::V6(ipv6_net)
                        };
                        debug!("current interface address: {ip_net:?}");
                        result.push(ip_net);
                    }
                }
                let _ = FreeMibTable(unicast_ip_address_table as *const _);
    ...
    fn refresh_ip_tables(&self) -> Result<()> {
        ...
        let allowed_ip_set = detail
            .peers
            .iter()
            .map(|p| p.allowed_ips.iter())
            .flatten()
            .cloned()
            .collect::<HashSet<_>>();

        unsafe {
            // flush route for this interface
            debug!("flush current adapter '{}' route", luid.Value);
            for family in [AF_INET, AF_INET6] {
                let mut ip_forward_table: *mut MIB_IPFORWARD_TABLE2 = ptr::null_mut();
                GetIpForwardTable2(family, &mut ip_forward_table as *mut _)
                    .context("list ip forward table")?;
                assert!(!ip_forward_table.is_null());
                for i in 0..(*ip_forward_table).NumEntries as usize {
                    let start = &mut (*ip_forward_table).Table[0] as *mut MIB_IPFORWARD_ROW2;
                    let ip_forward_row = start.add(i);
                    if (*ip_forward_row).InterfaceLuid.Value == luid.Value {
                        let _ = DeleteIpForwardEntry2(ip_forward_row);
                    }
                }
                let _ = FreeMibTable(ip_forward_table as *const _);
            }

            // setup route for the interface self
            let addresses = self.get_adapter_addresses()?;
            for address in addresses {
                let mut default_route = MIB_IPFORWARD_ROW2::default();
                InitializeIpForwardEntry(&mut default_route);

                default_route.InterfaceLuid = luid;
                match address {
                    IpNet::V4(v4) => {
                        default_route.DestinationPrefix.Prefix.Ipv4.sin_family = AF_INET;
                    ...
                    IpNet::V6(v6) => {
                        default_route.DestinationPrefix.Prefix.Ipv6.sin6_family = AF_INET6;
                ...
                if let Err(err) = CreateIpForwardEntry2(&default_route) {
                    if err != ERROR_OBJECT_ALREADY_EXISTS.into() {
                        bail!("faild set route for peer: {err:#}");
                    }
                }
            }

            // setup route
            debug!("setup current adapter '{}' route", luid.Value);
            for allowed_ip in allowed_ip_set {
                let mut default_route = MIB_IPFORWARD_ROW2::default();
                InitializeIpForwardEntry(&mut default_route);
